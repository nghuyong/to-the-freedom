...
def _print_summary(all_trades: list[dict]) -> None:
    ...
    header = (
        f"  {'股票':<8s} {'CD信号':<12s} {'首站上':<12s} {'买入日期':<12s} {'买入价':>10s}"
        f"  {'卖出日期':<12s} {'卖出价':>10s}"
        f"  {'收益%':>8s}  {'天数':>4s}  {'状态'}"
    )
    print(header)
    ...
    for t in sorted(all_trades, key=lambda x: x["buy_date"]):
        ...
        print(
            f"  {t['symbol']:<8s} {sig_dt:<12s} {fa_dt:<12s} {t['buy_date']:<12s} {t['buy_price']:>10.3f}"
            f"  {sell_dt:<12s} {t['sell_price']:>10.3f}"
            f"  {t['return_pct']:>+8.2f}  {t['hold_days']:>4d}  {status}"
        )
...
def main():
    ap = argparse.ArgumentParser(description="蓝色通道抄底策略回测")
    ...
    if args.symbol:
        ...
        output_path = Path(args.output) if args.output else (
            _RESULTS_DIR / f"backtest_{mode_tag}_{ts_short}.json"
        )
        header_desc = f"单股模式: {', '.join(s['symbol'] for s in stocks)}"
    else:
        stocks = load_top_stocks(args.market, args.top)
        output_path = Path(args.output) if args.output else (
            _RESULTS_DIR / f"backtest_{args.market}_top{args.top}_{ts_short}.json"
        )
        header_desc = f"{args.market.upper()} Top {args.top} ({len(stocks)} 只)"
