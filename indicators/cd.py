"""
CD 指标 — MACD 背离抄底/卖出信号。

基于 MACD 底背离和顶背离检测:
  - BUY_SIGNAL  (DXDX):   MACD 底背离确认信号（抄底）
  - SELL_SIGNAL (DBJGXC): MACD 顶背离确认信号（卖出）
  - BUY_CANCEL:           底背离消失
  - SELL_CANCEL:          顶背离消失
...
def compute_cd(
    ...
    -------
    pd.DataFrame
        追加了 DIFF, DEA, MACD_HIST, BUY_SIGNAL, SELL_SIGNAL,
        BUY_CANCEL, SELL_CANCEL 列的 DataFrame。
    """
    df = df.copy()
    ...
    dbl = (~ref_dbbl_1) & dbbl & _to_bool(diff > dea)
    ...
    dbjgxc = (~ref_dbjg_1) & dbjg  # SELL_SIGNAL
    ...
    wwwww = (dblxs | yyyyy) & (~dbbl)  # SELL_CANCEL

    # ── 输出 ──────────────────────────────────────────────────────────
    df["BUY_SIGNAL"] = dxdx
    df["SELL_SIGNAL"] = dbjgxc
    df["BUY_CANCEL"] = dxx
    df["SELL_CANCEL"] = wwwww
